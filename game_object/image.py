...
class Image(GameObject):
    ...
    def loop(self, screen, screen_pos):
        ...
        show_image(self.img, screen, pos + self.size / self.screen_factor * i, new_size=self.size,
                   center_image=center_image, angle=self.angle)
        if self.img_loop and pos.x + (i + 1) * self.size.x / self.screen_factor < screen_pos.x + get_screen_size().x:
            show_image(self.img, screen, pos + self.size / self.screen_factor * (i + 1), new_size=self.size,
                       center_image=center_image, angle=self.angle)
